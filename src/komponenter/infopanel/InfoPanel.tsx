...
const InfoPanelContainer: React.FC<{
    children: React.ReactNode;
    className?: string;
}> = ({children, className}) => {
    return <div className={"infopanel_container " + className}>{children}</div>;
};
...
const InfoPanel: React.FC<Props> = ({children, href, onClick, className}) => {
    let onClickHandler = onClick
        ? onClick
        : (event: any) => onClickLink(event, href);
